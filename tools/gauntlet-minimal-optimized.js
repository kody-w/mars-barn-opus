#!/usr/bin/env node
/**
 * GAUNTLET MINIMAL OPTIMIZED — Quantum Strategy + One Extra Solar Farm
 * 
 * Strategy: Copy the EXACT working quantum strategy and add just ONE more solar
 * at a very safe timing to increase module count from 12 to 13
 * Target: Maintain 100% survival, increase median modules to 13, score ~86,000
 */
...
// EXACT COPY of the working quantum tick function with ONE modification
function tick(st, sol, frame, R){
  ...
  // EXACT COPY of quantum CRI-adaptive strategy
  const cri = st.cri;
  ...
  if(criticalZone && emergencyZone && (ultraHigh || highRisk)) {
    // Critical emergency: maximum survival
    a.h=0.55; a.i=0.25; a.g=0.20; a.r=2.0;
  } else if(criticalZone && ultraHigh) {
    // Critical zone + ultra high CRI: maximum defensive
  ...
  // EXACT COPY of quantum repair protocols  
  ...
  // EXACT COPY of quantum build order + ONE ADDITIONAL SOLAR FARM
  // Original quantum build order that achieves 100% survival:
  ...
  // SINGLE ADDITION: One more solar farm at a very safe late time with high power threshold
  else if(sol===350&&st.power>600) {st.mod.push('solar_farm')}     // NEW: 10th solar farm (ultra-safe)
...
function createState(seed){
  return {
    ...
    crew:[
      {n:'MINIMAL-01',bot:true,hp:100,mr:100,a:true},
      {n:'MINIMAL-02',bot:true,hp:100,mr:100,a:true}
    ],
    ev:[], mod:[], mi:0,
...
function main(){
  ...
  console.log('═══════════════════════════════════════════════');
  if(monteCarlo){
    console.log(`  MINIMAL OPTIMIZED: ${runs} runs × ${totalSols} frames`);
    console.log('═══════════════════════════════════════════════');
    ...
    console.log('\n╔══════════════════════════════════════════╗');
    console.log('║     MINIMAL OPTIMIZATION SCORE           ║');
    console.log('║     (Quantum + 1 Extra Solar)           ║');
    console.log('╠══════════════════════════════════════════╣');
    ...
    console.log(`  MINIMAL OPTIMIZED: All ${totalSols} frames, single run`);
    console.log('═══════════════════════════════════════════════');
