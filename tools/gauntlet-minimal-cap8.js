...
function tick(st, sol, frame, R){
  ...
  // SMART CAP-8 OPTIMIZATION: Build for survival, but prioritize scoring-efficient modules FIRST
  // Strategy: Build the highest-scoring module types early, then add survival redundancy
  // Since only first 8 modules count for scoring, make sure they're the right types
  
  // Phase 1: HIGH-SCORING MODULES FIRST (these count toward the 8-module cap)
  if(sol===2&&st.power>12)         {st.mod.push('solar_farm')}     // 1st scored - immediate power
  else if(sol===5&&st.power>20)    {st.mod.push('repair_bay')}     // 2nd scored - efficiency multiplier
  else if(sol===8&&st.power>30)    {st.mod.push('solar_farm')}     // 3rd scored - more power
  else if(sol===12&&st.power>40)   {st.mod.push('repair_bay')}     // 4th scored - exponential efficiency
  else if(sol===16&&st.power>50)   {st.mod.push('isru_plant')}     // 5th scored - O2/H2O critical
  else if(sol===20&&st.power>60)   {st.mod.push('greenhouse_dome')} // 6th scored - food production
  else if(sol===26&&st.power>75)   {st.mod.push('water_extractor')} // 7th scored - water backup
  else if(sol===32&&st.power>90)   {st.mod.push('radiation_shelter')} // 8th scored - complete 6 unique types
  
  // Phase 2: SURVIVAL REDUNDANCY (these don't count for scoring but are needed for survival)
  else if(sol===38&&st.power>105)  {st.mod.push('solar_farm')}     // Extra power
  else if(sol===45&&st.power>125)  {st.mod.push('solar_farm')}     // More power
  else if(sol===52&&st.power>145)  {st.mod.push('repair_bay')}     // More efficiency
  else if(sol===60&&st.power>170)  {st.mod.push('solar_farm')}     // Even more power
  else if(sol===70&&st.power>200)  {st.mod.push('repair_bay')}     // More repairs
  else if(sol===80&&st.power>235)  {st.mod.push('isru_plant')}     // More ISRU
  else if(sol===92&&st.power>275)  {st.mod.push('greenhouse_dome')} // More food
  else if(sol===105&&st.power>320) {st.mod.push('water_extractor')} // More water
  else if(sol===120&&st.power>370) {st.mod.push('solar_farm')}     // Keep building power
  else if(sol===135&&st.power>420) {st.mod.push('repair_bay')}     // Keep building efficiency
  else if(sol===152&&st.power>480) {st.mod.push('solar_farm')}     // More power redundancy
  
  // Continue building for survival (following original successful pattern)
