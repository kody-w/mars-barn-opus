...
function tick(st, sol, frame, R){
  ...
  if(sol===2&&st.power>12&&st.mod.length<8)         {st.mod.push('solar_farm')}     // Immediate start
  else if(sol===5&&st.power>20&&st.mod.length<8)    {st.mod.push('solar_farm')}     // Ultra-rapid acceleration
  else if(sol===8&&st.power>30&&st.mod.length<8)    {st.mod.push('solar_farm')}     // Power foundation
  else if(sol===12&&st.power>40&&st.mod.length<8)   {st.mod.push('solar_farm')}     // Early surplus
  else if(sol===16&&st.power>50&&st.mod.length<8)   {st.mod.push('solar_farm')}     // 5th solar even earlier
  // Revolutionary ultra-early repair investment 
  else if(sol===20&&st.power>60&&st.mod.length<8)   {st.mod.push('repair_bay')}     // Ultra-early repair (5 sols earlier)
  // Continued aggressive solar buildup
  else if(sol===26&&st.power>75&&st.mod.length<8)   {st.mod.push('solar_farm')}     // 6th solar
  else if(sol===32&&st.power>90&&st.mod.length<8)   {st.mod.push('solar_farm')}     // 7th solar
  else if(sol===38&&st.power>105&&st.mod.length<8)  {st.mod.push('repair_bay')}     // 8th MODULE - FINAL! (2nd repair bay)
  // NO MORE MODULES AFTER 8 DUE TO SCORING CAP
