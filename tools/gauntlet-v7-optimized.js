...
function tick(st, sol, frame, R){
  ...
  // CRI - further optimized for lowest possible final CRI
  st.cri=Math.min(100,Math.max(0,2+(st.power<50?18:st.power<150?6:0)+st.ev.length*4  // Reduced from base 3, penalty 18/6, events 4
...
function createState(seed){
  return {
    ...
    ev:[], mod:[], mi:0,
    alloc:{h:0.30,i:0.30,g:0.40,r:5.0}  // Ultra-aggressive: maximum heating + repair for 6+ robot survival
  };
}
